...
class UserController extends Controller
{
    ...
    public function customeValidate($data, $mode)
    {
        ...
        if ($validator->fails()) {
            $errorRedirectUrl = "doctor/add";
            if ($mode == "edit") {
                $errorRedirectUrl = "doctor/edit/" . $data['id'];
            }
    ...
    public function store(request $request)
    {
        ...
        try{
            ...
            return redirect('doctor/add')->withInput();
        ...
        if ($adduser) {
            ...
            $request->session()->flash('alert-success', __('app.default_add_success',["module" => __('app.doctor')]));
            return redirect('doctor/');
        } else {
            $request->session()->flash('alert-danger', __('app.default_error',["module" => __('app.doctor'),"action"=>__('app.add')]));
            return redirect('doctor/add')->withInput();
        }
    }
    ...
    public function update(request $request)
    {
        ...
        try{
            ...
            DB::rollback();
            $request->session()->flash('alert-danger', $e->getMessage());
            return redirect('doctor/edit/' . $request->get('id'))->withInput();
        }

        if ($updateuser) {
            $request->session()->flash('alert-success', __('app.default_edit_success',["module" => __('app.doctor')]));
            return redirect('doctor/');
        }

        $request->session()->flash('alert-danger', __('app.default_error',["module" => __('app.doctor'),"action"=>__('app.update')]));
        return redirect('doctor/edit/' . $request->get('id'))->withInput();
